...
<body>
  ...
  <div id="wrapper">
    ...
    <?php
      $user=Query::affiche('etudiant',$_SESSION['id'],'id');
  
      // confirmation du vote
      $requette6=$bdd->prepare("SELECT * FROM vote WHERE id_etudiant=?");
      $requette6->execute(array($_SESSION['id']));
      $confirmation=$requette6->rowCount();
    ?>
    <?php include('admin/include-admin/flash.php'); ?>
    <center><h6 class="alert alert-info"> <i class="fa fa-user"></i> Bienvenue <b><?= $user->nom."  ".$user->prenom;  ?></b></h6></center>
    ...
    <div class="alert alert-success">
      ...
      <strong>Merci d'avoir voter pour</strong> Votre candidat
    </div>
  <?php } ?>
    <section id="content">
      <div class="container">
        ...
        <div class="row">
          ...
          ?>
          <div class="span3" style="border: 1px solid black; padding: 15px;">

            <img src="admin/candidat/photo/<?php echo $candidat['photo']; ?>" alt="" class="img-polaroid" />

            <center>
              <h5 style=" text-align: center; font-weight: bold;" ><?php echo $candidat['prenom']."  ".$candidat['nom'];; ?></h5></center>
            ...
            <div class="roles">
              ...
              <form action="" method="POST">
                <input type="hidden" value="<?php echo $candidat['id'];?>" name="id">
              <center><button title="<?php if($confirmation==1){echo "Vous avez déjà voté";}else{echo "VOTEZ";} ?>" <?php if($confirmation==1 OR $jours<=0 OR $lancement==0){echo "disabled";} ?>  type="submit"  name="vote" class="btn btn-lg btn-block btn-primary"> VOTEZ</button></center>
              <hr/>
              </form>
            </div>
          </div>
